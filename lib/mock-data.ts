export type Platform = "tiktok" | "instagram";

export interface Place {
  id: string;
  name: string;
  neighborhood: string;
  category: string;
  creator: string;
  platform: Platform;
  saved: boolean;
  rating: number;
  priceLevel: number;
  description: string;
  gradient: string;
  lat: number;
  lng: number;
}

export interface PlaceList {
  id: string;
  name: string;
  emoji: string;
  placeCount: number;
  gradient: string;
  collaborators: string[];
  isShared: boolean;
  isPrivate?: boolean;
}

export interface Challenge {
  id: string;
  brand: string;
  title: string;
  description: string;
  reward: string;
  progress: number;
  total: number;
  color: string;
  badgeEmoji: string;
  daysLeft: number;
}

export const PLACES: Place[] = [
  {
    id: "1",
    name: "Sushi Nakazawa",
    neighborhood: "West Village",
    category: "Restaurants",
    creator: "@mayanyc",
    platform: "tiktok",
    saved: true,
    rating: 4.9,
    priceLevel: 4,
    description: "Omakase by Jiro's protégé. 20-course meal that's worth every penny.",
    gradient: "from-indigo-900 to-purple-900",
    lat: 40.7308,
    lng: -74.0027,
  },
  {
    id: "2",
    name: "Raoul's",
    neighborhood: "SoHo",
    category: "Restaurants",
    creator: "@nycfoodtours",
    platform: "instagram",
    saved: false,
    rating: 4.7,
    priceLevel: 3,
    description: "Old-school French bistro. The steak au poivre is legendary.",
    gradient: "from-rose-900 to-pink-900",
    lat: 40.7248,
    lng: -74.0018,
  },
  {
    id: "3",
    name: "Joe's Pizza",
    neighborhood: "West Village",
    category: "Restaurants",
    creator: "@nycfoodtours",
    platform: "instagram",
    saved: true,
    rating: 4.7,
    priceLevel: 1,
    description: "NYC's most iconic slice. No frills, just perfect pizza since 1975.",
    gradient: "from-red-900 to-orange-900",
    lat: 40.7325,
    lng: -74.0024,
  },
  {
    id: "4",
    name: "John's of Bleecker Street",
    neighborhood: "West Village",
    category: "Restaurants",
    creator: "@mayanyc",
    platform: "tiktok",
    saved: true,
    rating: 4.6,
    priceLevel: 2,
    description: "Coal-fired brick oven pizza since 1929. Cash only and worth the wait.",
    gradient: "from-orange-900 to-yellow-900",
    lat: 40.7312,
    lng: -74.0031,
  },
  {
    id: "5",
    name: "Rosemary's",
    neighborhood: "West Village",
    category: "Restaurants",
    creator: "@nycfoodtours",
    platform: "instagram",
    saved: false,
    rating: 4.5,
    priceLevel: 2,
    description: "Farm-to-table Italian with a rooftop garden. Sunday brunch is a must.",
    gradient: "from-green-900 to-emerald-900",
    lat: 40.7338,
    lng: -74.0019,
  },
  {
    id: "6",
    name: "Beacon's Closet",
    neighborhood: "West Village",
    category: "Shopping",
    creator: "@vintageviv",
    platform: "instagram",
    saved: false,
    rating: 4.5,
    priceLevel: 1,
    description: "The best thrift and vintage spot in the city. Rotating stock, great prices.",
    gradient: "from-purple-900 to-violet-900",
    lat: 40.7376,
    lng: -73.9980,
  },
  {
    id: "7",
    name: "Blank Street Coffee",
    neighborhood: "Williamsburg",
    category: "Coffee",
    creator: "@morningnyc",
    platform: "tiktok",
    saved: true,
    rating: 4.5,
    priceLevel: 2,
    description: "Matcha drinks and cozy vibes. The pistachio latte is unreal.",
    gradient: "from-green-900 to-teal-900",
    lat: 40.7183,
    lng: -73.9566,
  },
  {
    id: "8",
    name: "Toby's Estate",
    neighborhood: "Williamsburg",
    category: "Coffee",
    creator: "@traveljunkie",
    platform: "instagram",
    saved: true,
    rating: 4.6,
    priceLevel: 2,
    description: "Serious specialty coffee in a gorgeous industrial space.",
    gradient: "from-amber-900 to-yellow-900",
    lat: 40.7191,
    lng: -73.9572,
  },
  {
    id: "9",
    name: "Cure Thrift Shop",
    neighborhood: "East Village",
    category: "Shopping",
    creator: "@vintageviv",
    platform: "instagram",
    saved: false,
    rating: 4.4,
    priceLevel: 1,
    description: "Charity thrift shop with amazing finds. All proceeds go to HIV/AIDS research.",
    gradient: "from-pink-900 to-rose-900",
    lat: 40.7264,
    lng: -73.9815,
  },
  {
    id: "10",
    name: "The Bar at Bemelmans",
    neighborhood: "Upper East Side",
    category: "Bars",
    creator: "@luxurynyc",
    platform: "instagram",
    saved: false,
    rating: 4.8,
    priceLevel: 4,
    description: "Classic New York glamour. The murals, the jazz, the martinis.",
    gradient: "from-amber-900 to-yellow-900",
    lat: 40.7745,
    lng: -73.9630,
  },
  {
    id: "11",
    name: "McNally Jackson",
    neighborhood: "Nolita",
    category: "Shopping",
    creator: "@bookwormnyc",
    platform: "instagram",
    saved: true,
    rating: 4.6,
    priceLevel: 2,
    description: "The best indie bookshop in the city. Great staff picks.",
    gradient: "from-sky-900 to-blue-900",
    lat: 40.7232,
    lng: -73.9960,
  },
  {
    id: "12",
    name: "Joe Coffee",
    neighborhood: "West Village",
    category: "Coffee",
    creator: "@coffeecritic",
    platform: "tiktok",
    saved: false,
    rating: 4.4,
    priceLevel: 2,
    description: "Single origin pour overs in a neighborhood gem.",
    gradient: "from-orange-900 to-red-900",
    lat: 40.7331,
    lng: -74.0025,
  },
];

export const LISTS: PlaceList[] = [
  {
    id: "1",
    name: "NYC Eats 🔥",
    emoji: "🍣",
    placeCount: 11,
    gradient: "from-red-800 to-orange-900",
    collaborators: ["S"],
    isShared: false,
    isPrivate: true,
  },
  {
    id: "2",
    name: "Tokyo Trip 🗾",
    emoji: "✈️",
    placeCount: 8,
    gradient: "from-indigo-800 to-purple-900",
    collaborators: ["S", "A", "Sa"],
    isShared: true,
    isPrivate: false,
  },
  {
    id: "3",
    name: "WFH Spots ☕",
    emoji: "💻",
    placeCount: 5,
    gradient: "from-green-900 to-emerald-950",
    collaborators: ["S"],
    isShared: false,
    isPrivate: false,
  },
  {
    id: "4",
    name: "Date Night NYC",
    emoji: "🕯️",
    placeCount: 7,
    gradient: "from-purple-600 to-pink-600",
    collaborators: ["S", "M"],
    isShared: true,
    isPrivate: false,
  },
];

export const TOKYO_PLACES = [
  {
    id: "t1",
    name: "Ichiran Ramen",
    neighborhood: "Shibuya",
    creator: "@mayanyc",
    saved: true,
    emoji: "🍜",
  },
  {
    id: "t2",
    name: "Senso-ji Temple",
    neighborhood: "Asakusa",
    creator: "Added by Alex",
    saved: false,
    emoji: "⛩️",
  },
  {
    id: "t3",
    name: "Ace Hotel Tokyo",
    neighborhood: "Shinjuku",
    creator: "Added by Sarah",
    saved: false,
    emoji: "🏨",
  },
  {
    id: "t4",
    name: "Tsukiji Outer Market",
    neighborhood: "Tsukiji",
    creator: "@traveljunkie",
    saved: true,
    emoji: "🍱",
  },
];

export const CHALLENGES: Challenge[] = [
  {
    id: "1",
    brand: "Resy × Come With Me",
    title: "NYCs Best Tables",
    description: "Save 5 Resy-listed restaurants to your map and get priority access to impossible reservations.",
    reward: "🎁 Priority Resy access + $50 dining credit",
    progress: 3,
    total: 5,
    color: "#CAFF33",
    badgeEmoji: "🍽️",
    daysLeft: 4,
  },
  {
    id: "2",
    brand: "Bluestone Lane",
    title: "The Morning Ritual",
    description: "Check in at 3 Bluestone Lane cafés across NYC and discover your new go-to morning spot.",
    reward: "🎁 Free week of flat whites",
    progress: 1,
    total: 3,
    color: "#CAFF33",
    badgeEmoji: "☕",
    daysLeft: 7,
  },
  {
    id: "3",
    brand: "Depop × Come With Me",
    title: "Vintage NYC Trail",
    description: "Save 8 vintage & thrift spots in NYC to unlock exclusive Depop seller perks.",
    reward: "🎁 0% selling fees for 30 days",
    progress: 5,
    total: 8,
    color: "#CAFF33",
    badgeEmoji: "🛍️",
    daysLeft: 10,
  },
];// force redeploy
